...
inception_layers = ['Conv2d_1a_3x3',
                    'Conv2d_2a_3x3',
                    'Conv2d_2b_3x3',
                    'MaxPool_3a_3x3',
                    'Conv2d_3b_1x1',
...
class BoxInceptionResnet(BoxNetwork):
	def __init__(self, inputs, nCategories, name="BoxNetwork", weightDecay=0.00004, freezeBatchNorm=False, reuse=False, isTraining=True, trainFrom=None, hardMining=True):
		...
		if trainFrom == "-1":
			...
			try :
		        trainFrom = inception_layers[int(trainFrom)]
	        except:
				print("Incorrect indice for inceptionlayer to start training From)
				print("corresponding inception layers for each indice is : \n")
				for i in range(len(inception_layers)):
					print(i,"  ==>  ",inception_layers[i])
				print("\nNow using default value [end]!")
	            trainFrom = None
